...
@pytest.mark.asyncio
async def test_agent_query(agent_app: AgentEngineApp) -> None:
    """Integration test for the non-streaming query method, with and without user_id."""
    import json
    ...
    # 1. Query with user_id
    result = await agent_app.query(message=message, user_id="test")
    assert result is not None

    # 2. Query without user_id (should default to "default-user")
    result_default = await agent_app.query(message=message)
